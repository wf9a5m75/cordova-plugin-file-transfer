...
このプラグインを使用すると、ファイルをアップロードしたりダウンロードすることができます。

プラグインはグローバルに`FileTransfer`と`FileUploadOptions`コンストラクタを定義します。

これらは`deviceready`イベントの後から利用することができます。
...
*`onprogress`と`about()`は未サポート

** `onprogress` は未サポート

*** `onprogress`はuploadメソッドに対して部分的にサポート。Windowsによる制限のため、`onprogress`は空の進捗イベントとして呼び出されます。

# ファイル転送

`FileTransfer`オブジェクトはHTTPマルチパートPostリクエストを使用したアップロードとダウンロードをする方法を提供します。

## プロパティ

*   **onprogress**: 新しいデータのチャンクが転送されるたびにコールバック関数を呼び出します。コールバック関数には `ProgressEvent`が渡されます。

## メソッド

*   **upload**: サーバーにファイルを送信します。

*   **download**: サーバーからファイルをダウンロードします。
...
*   **fileURL**: デバイス上でファイルを示すファイルシステムURL。下位互換性のため、このパラメータにはデバイス上のファイルのフルパスも可能。（下部の[後方互換性メモ] を参照）

*   **server**: `encodeURI()`によってエンコードされた、ファイルを受け取るサーバーのURL

*   **successCallback**: `FileUploadResult`オブジェクトを受け取るコールバック関数

*   **errorCallback**: もし処理中にエラーが発生した際に`FileUploadResult`を受け取るコールバック関数。`FileTransferError`オブジェクトと一緒に呼び出されます。
